...
class User(BaseModel, db.Model):
    ...
    houses = db.relationship("House", backref="user")  # 用户发布的房屋 一的里面定义relationship
    orders = db.relationship("Order", backref="user")  # 用户下的订单 一的里面定义relationship
    ...
    def password(self, passwd):
        ...
        self.password_hash = generate_password_hash(passwd) # sha256加密

    def check_password(self, passwd):
        """检查密码的正确性"""
        return check_password_hash(self.password_hash, passwd) # 传入的是加密后的密码和用户输入的密码,会自动加密
...
class House(BaseModel, db.Model):
    ...
    facilities = db.relationship("Facility", secondary=house_facility)  # 房屋的设施  多对多核心
